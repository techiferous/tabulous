...
describe 'subtabs' do
  ...
  it 'should be selectable' do
    visit '/'
    click_link 'Galaxies'
    expect(page).to have_content(/listing.+elliptical.+galaxies/i)
    click_link 'Spiral Galaxies'
    expect(page).to have_content(/listing.+spiral.+galaxies/i)
    click_link 'Lenticular Galaxies'
    expect(page).to have_content(/listing.+lenticular.+galaxies/i)
  end
end
