class Planets::RoguePlanetsController < ApplicationController
  ...
  # GET /planets/rogue_planets.xml
  def index
    @planets_rogue_planets = Planets::RoguePlanet.all

    respond_to do |format|
      format.html # index.html.erb
      format.xml  { render :xml => @planets_rogue_planets }
    end
  end

  # GET /planets/rogue_planets/1
  # GET /planets/rogue_planets/1.xml
  def show
    @planets_rogue_planet = Planets::RoguePlanet.find(params[:id])

    respond_to do |format|
      format.html # show.html.erb
      format.xml  { render :xml => @planets_rogue_planet }
    end
  end

  # GET /planets/rogue_planets/new
  # GET /planets/rogue_planets/new.xml
  def new
  ...
  # POST /planets/rogue_planets.xml
  def create
    ...
    respond_to do |format|
      if @planets_rogue_planet.save
        ...
        format.xml  { render :xml => @planets_rogue_planet, :status => :created, :location => @planets_rogue_planet }
      else
        format.html { render :action => "new" }
        format.xml  { render :xml => @planets_rogue_planet.errors, :status => :unprocessable_entity }
      end
    end
  end

  # PUT /planets/rogue_planets/1
  # PUT /planets/rogue_planets/1.xml
  def update
    @planets_rogue_planet = Planets::RoguePlanet.find(params[:id])
  ...
  # DELETE /planets/rogue_planets/1.xml
  def destroy
    @planets_rogue_planet = Planets::RoguePlanet.find(params[:id])
